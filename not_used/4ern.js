...
const waste_code_explanations = ['Показник загального утворення відходів (ПЗУВ) - критерій обсягу утворення відходів, що розраховується за формулою Пзув = 5000 х М1 + 500 х М2 + 50 х М3 + 1 х М4, де М1, М2, М3, М4 - маса в тонах відходів 1, 2, 3 та 4 класів небезпеки відповідно',
    'Загальна сума кількості відходів в тоннах, незалежно від класу небезпеки','Надзвичайно небезпечні речовини','Високонебезпечні речовини','Помірно небезпечні речовини','Безпечні речовини']
...
function clearInputFunction(d) {
    var div = document.getElementsByClassName("name_input")[0];
...
function filterInputFunction() {
    ...
    input = document.getElementsByClassName("name_input__field")[0];
    filter = input.value.toUpperCase();
    div = document.getElementsByClassName("name_input")[0];
...
d3.selectAll('a').nodes().forEach(function (elem) {
    elem.addEventListener("click", function () {
        d3.select('.name_input__field')
            .value = this.text;
    })
})


var display_buttons = d3.selectAll('.stat-selector__button')
...
let main = function () {
        ...
        return Promise.all(value
            ...
            .map(function (results) {
                for (let j = 0; j < sel_year_values.length; j++) {
                    return [results.Name, Math.round(results[sel_year_values[j] + '_' + sel_waste.options[sel_waste.selectedIndex].value] * 1000) / 1000, results.EDRPOU, results.KOATUU];
                }
            })
            .sort(function(a, b){
                return b[1]-a[1]
    ...
    coords.then(function (data) {
        ...
        var xScale = d3.scaleBand()
            ...
            .domain(data.slice(parseInt(inp_num[0].value) - 1, parseInt(inp_num[0].value) - 1 + parseInt(inp_num[1].value)).map((s, i) => (parseInt(inp_num[0].value) + i).toString() + '. ' + s[0]))
            .padding(0.2);

        d3.select(".scroller_text")
            .text(inp_num[0].value + '-' + (parseInt(inp_num[0].value) + parseInt(inp_num[1].value) - 1).toString() + ' з ' + inp_num[0].max)

        d3.select('.name_input__list')
            .selectAll("*")
            .remove()
        ...
        d3.selectAll('a').nodes().forEach(function (elem) {
            elem.addEventListener("click", function () {
                document.getElementsByClassName('name_input__field')[0].value = this.text;
                // console.log(d3.select('.name_input__field').value, this.text)
                filterInputFunction();
        ...
        barGroups
            .append('rect')
            .attr('class', 'bar')
            .attr('x', (s, i) => xScale((parseInt(inp_num[0].value) + i).toString() + '. ' + s[0]))
            .attr('y', (s) => yScale(s[1]))
        ...
        barGroups
            .on('mouseenter', function (actual, i) {
                ...
                d3.select(this)
                    .transition()
                    .duration(300)
                    .attr('x', (s, i) => xScale((parseInt(inp_num[0].value) + i).toString() + '. ' + s[0]) - 5)
                ...
                barGroups.append('text')
                    ...
                    .attr('x', (s, i) => xScale((parseInt(inp_num[0].value) + i).toString() + '. ' + s[0]) + xScale.bandwidth() / 2)
            ...
            .on('mouseleave', function () {
                ...
                d3.select(this)
                    .transition()
                    .duration(300)
                    .attr('x', (a, i) => xScale((parseInt(inp_num[0].value) + i).toString() + '. ' + a[0]))
                    .attr('width', xScale.bandwidth())
                    .select(".bar")
                    .attr('opacity', 1);
        ...
        barGroups
            .append('text')
            .attr('class', 'value')
            .attr('x', (a, i) => xScale((parseInt(inp_num[0].value) + i).toString() + '. ' + a[0]) + xScale.bandwidth() / 2)
            .attr('y', (a) => yScale(a[1]) + 30 > height ? yScale(a[1]) - 5 : yScale(a[1]) + 30)
            .attr('text-anchor', 'middle')
            .text((a) => `${a[1]}`)
        ...
        // console.log(sel_year_values, function (){
        //     const d = d3.selectAll('.ss-value-text')
        //     let vals = []
        //     d.each(function () {
        //         vals.push(this.innerText)
        ...
        function display () {
            ...
            // console.log(nf, numBars)
            inp_num[0].value = nf + 1;
            // inp_num[1].value = nf + parseInt(inp_num[1].value);

            sel_waste.click();

//                 new_data = data.slice(nf, nf + numBars);
//
//                 xScale.domain(new_data.map(function (d) { return d[0]; }));
//                 chart.select(".x.axis").call(d3.axisBottom(xScale));
//
//                 rects = d3.select(".bars_grouper").selectAll("rect")
...
//     .range([0, 100]);
//
// console.log(linearScale(3555000));